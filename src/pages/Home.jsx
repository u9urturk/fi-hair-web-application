...
export default function Home() {
  ...
  return (
    <div style={{ backgroundImage: `url(${bgImage})` }} className='max-w-[1400px] flex justify-between relative w-full h-screen  bg-blend-multiply p-4 mx-auto bg-center bg-cover'>
      <button type='button' className='flex gap-x-2 items-center justify-center   rounded-lg w-48 h-10 bg-brand-color' disabled={true}>
        <BiAnalyse className='text-gray-300 animate-spin' size={20}></BiAnalyse>
        <p className='  text-gray-300 font-light'   >Yapım aşamasında ...</p>
      </button>
      <button onClick={() => { setActive(true) }} className='flex  items-center justify-center transition-all  hover:scale-105  text-gray-300  rounded-lg w-48 h-10 bg-brand-color'>Developer Area</button>
